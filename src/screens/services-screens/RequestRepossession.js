...
const SingleSearch = React.lazy(() => import('../../components/request-repossession/SingleSearch'));
const BulkRequest = React.lazy(() => import('../../components/request-repossession/BulkRequest'));

function RequestRepossession() {
  ...
  const [repossessionDetails, setRepossessionDetails] = useState('');
  ...
  const removeDocument = (name) => {
    const documentsAfterRemoval = repossessionDetails.supportingDocuments.filter((item) => {
          ...
          <Steps className="p-4" lang={i18n.language}>
            ...
            {activeSearch == 'single' ? (
              <SingleSearch
                ...
                setRepossessionDetails={setRepossessionDetails}
...
export default hot(module)(RequestRepossession);
